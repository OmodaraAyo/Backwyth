export const otherSignInOptions = [
  {
    name: "Google",
    icon: "https://img.icons8.com/color/48/000000/google-logo.png",
  },
  {
    name: "Facebook",
    icon: "https://img.icons8.com/color/48/000000/facebook-new.png",
  },
  {
    name: "Twitter",
    icon: "https://img.icons8.com/color/48/000000/twitter--v1.png",
  },
];

export const categories = [
  { value: "finance", label: "Finance" },
  { value: "healthCare", label: "Healthcare" },
  { value: "technology", label: "Technology" },
  { value: "social", label: "Social" },
  { value: "education", label: "Education" },
  { value: "ecommerce", label: "Ecommerce" },
];

export const navBarItems = [
  { page: "Home", path: "/" },
  { page: "Pricing", path: "/pricing" },
  { page: "About Us", path: "/about-us" },
  { page: "FAQs", path: "/faqs" },
  { page: "Doc", path: "/documentation" },
  { page: "Dashboard", path: "/dashboard" },
];

export const dashboardChildren = [
  { page: "Profile", path: "/profile" },
  { page: "Menus", path: "/menus" },
  { page: "Feedback", path: "/feedback" },
  { page: "Notifications", path: "/notifications" },
  { page: "Change Password", path: "/change-password" },
  { page: "Support", path: "/support" },
  { page: "Settings", path: "/settings" },
];

export const changePasswordFormData = [
  {
    label: "Old Password",
    name: "oldPassword",
    placeHolder: "Current password",
  },
  {
    label: "New Password",
    name: "newPassword",
    placeHolder: "9+ strong character",
  },
  {
    label: "Confirm Password",
    name: "confirmPassword",
    placeHolder: "9+ strong character",
  },
];

export const keyFeaturesData = [
  {
    title: "Instant USSD Integration",
    details:
      "No apps, no internet, just seamless, reliable connectivity across all networks.",
  },
  {
    title: "Scalable & Secure APIs",
    details: "Grow without limits while ensuring enterprise-grade security.",
  },
  {
    title: "Real-Time Connectivity",
    details:
      "Instant, reliable connectivity ensuring real-time data exchange and seamless user interactions.",
  },
  {
    title: "Effortless Management",
    details:
      "effortlessly configure menus, track usage, and optimize performance in real time.",
  },
];
